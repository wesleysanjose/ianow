...
class ChromaProcessor:
    ...
    def convert_from_docs(self, docs, embeddings=None):
        ...
        try:
            ...
            log.error(
                f'Unexpected error occurred while creating vectorstore: {e}')
            self.vectorstore = None
            raise e
    ...
    def load_from_disk(self):
        ...
        try:
            ...
            log.info(
                f'Vectorstore loaded with {len(self.vectorstore)} vectors')
        except FileNotFoundError as fnf_error:
            ...
            log.error(
                f'Unexpected error occurred while loading vectorstore: {e}')
            self.vectorstore = None
            raise e
    ...
    def save(self):
        ...
        if self.vectorstore:
            try:
                ...
                log.error(
                    f'Unexpected error occurred while saving vectorstore: {e}')
                raise e
        else:
            log.error(
                'No vectorstore to save. Please ensure the vectorstore is loaded or created before saving.')
